...
use Fxtm\CopyTrading\Tests\Entities\Mt5SymbolSession;

class Mt5SymbolSessionFixture extends BaseFixture
{
    ...
    public function load(ObjectManager $manager)
    {
        foreach ($this->getFixtureData() as $elem) {
            $symbolSession = new Mt5SymbolSession();
            ...
            $symbolSession->setOpenHours($elem[self::KEY_OPEN_HOURS] ?? self::DEFAULT_OPEN_HOURS);
            $symbolSession->setOpenMinutes($elem[self::KEY_OPEN_MINUTES] ?? self::DEFAULT_OPEN_MINUTES);
            $symbolSession->setCloseHours($elem[self::KEY_CLOSE_HOURS] ?? self::DEFAULT_CLOSE_HOURS);
            $symbolSession->setCloseMinutes($elem[self::KEY_CLOSE_MINUTES] ?? self::DEFAULT_CLOSE_MINUTES);
