...
jQuery(function ($) {
    if(Cookies.get('shortform')){
        var fields = JSON.parse(Cookies.get('shortform'));
        console.log(fields);
        for(var i = 0; i < fields.length; i++){
            ...
            if (fieldName==='phone_prefix'){
                // fieldVal = fieldVal.substr(1,fieldVal.length);
            }
            if (fieldVal.length !== 0){
                field.val(fieldVal);
                console.log(fieldVal.length, fieldName, fieldVal);
                if (field.parent().hasClass('field_wrap')) {
                    field.parent().addClass('has-label');
                }
            }
        }
        // $('#agree_pp').prop("checked", true).closest('.field_wrap').addClass('active');
        var lang = $('html').attr('lang');
        var btnText = 'Continue Registration';
        switch (lang){
            case 'ru':
                btnText = 'Продолжить регистрацию';
        }
    }
            ...
            if ($('#shortForm').parsley().validate()) {
                ...
                $.ajax({
                    ...
                    if(response['response'].messageCode == '5'){
                        ...
                        Cookies.set('shortform', JSON.stringify(formSerialized), {expires: 365});
    ...
    window.Parsley.on('field:error', function() {
        Cookies.set('shortform', JSON.stringify($('#shortForm').find('input,select').serializeArray()), {expires: 365});
    ...
    $(document).ready(function () {
        $('#parsley-id-multiple-agree_pp').remove();
    });
});
